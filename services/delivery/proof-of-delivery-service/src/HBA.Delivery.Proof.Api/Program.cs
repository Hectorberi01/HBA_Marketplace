using HBA.ProofOfDelivery.Api.Endpoints;
using HBA.ProofOfDelivery.Api.Grpc;
using HBA.ProofOfDelivery.Infrastructure;
...
builder.Services.AddProofOfDeliveryInfrastructure();
builder.AddHbaGrpc();
...
app.MapGet("/health/live", () => Results.Ok(new { status = "live", service = "HBA.ProofOfDelivery" })).AllowAnonymous();
...
app.MapGet("/health/ready", () => Results.Ok(new { status = "ready", service = "HBA.ProofOfDelivery" })).AllowAnonymous();

app.MapProofEndpoints();
app.MapInternalGrpcService<ProofGrpcService>();
