...
namespace HBA.Delivery.Pricing.Infrastructure.Messaging.Kafka.Processors;
...
public sealed class OutboxProcessor : BackgroundService
{
    ...
    private static readonly string ModuleName = typeof(DeliveryPricingDbContext).Name.Replace("DbContext", string.Empty);
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                _logger.LogError(ex, "Erreur lors du traitement de l'outbox de {Context}", typeof(DeliveryPricingDbContext).Name);
    ...
    private async Task ProcessBatchAsync(CancellationToken cancellationToken)
    {
        ...
        var dbContext = scope.ServiceProvider.GetRequiredService<DeliveryPricingDbContext>();
    ...
    private void HandleFailure(OutboxMessage message, Exception exception)
    {
        ...
        if (deadLettered)
        {
            ...
            _logger.LogCritical(
                ...
                typeof(DeliveryPricingDbContext).Name, message.Id, message.Type, message.AttemptCount);
