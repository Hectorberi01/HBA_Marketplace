using HBA.Dispatch.Api.Endpoints;
using HBA.Dispatch.Api.Grpc;
...
builder.Services.AddDispatchInfrastructure();
builder.AddHbaGrpc();
...
app.MapGet("/health/live", () => Results.Ok(new { status = "live", service = "HBA.Dispatch" })).AllowAnonymous();
...
app.MapGet("/health/ready", () => Results.Ok(new { status = "ready", service = "HBA.Dispatch" })).AllowAnonymous();

app.MapDispatchEndpoints();
app.MapInternalGrpcService<DispatchGrpcService>();
