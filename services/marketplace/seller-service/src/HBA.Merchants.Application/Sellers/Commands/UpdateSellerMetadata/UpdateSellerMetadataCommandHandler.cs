...
internal sealed class UpdateSellerMetadataCommandHandler : ICommandHandler<UpdateSellerMetadataCommand>
{
    private readonly ISellerRepository _sellerRepository;
    private readonly ISellerUnitOfWork _unitOfWork;

    public UpdateSellerMetadataCommandHandler(ISellerRepository sellerRepository, ISellerUnitOfWork unitOfWork)
    {
        _sellerRepository = sellerRepository;
        _unitOfWork = unitOfWork;
    }

    public async Task<Result> Handle(UpdateSellerMetadataCommand command, CancellationToken cancellationToken)
    {
        ...
        var result = seller.UpdateMetadata(command.Metadata);
        if (result.IsFailure)
        {
            return result;
        }
