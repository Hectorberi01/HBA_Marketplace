...
namespace HBA.FoodCarts.Infrastructure.Messaging.Kafka.Processors;
...
public sealed class OutboxProcessor : BackgroundService
{
    ...
    /// </summary>
    private static readonly string ModuleName = typeof(FoodCartDbContext).Name.Replace("DbContext", string.Empty);
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                _logger.LogError(ex, "Erreur lors du traitement de l'outbox de {Context}", typeof(FoodCartDbContext).Name);
    ...
    private async Task ProcessBatchAsync(CancellationToken cancellationToken)
    {
        ...
        var dbContext = scope.ServiceProvider.GetRequiredService<FoodCartDbContext>();
    ...
    private void HandleFailure(OutboxMessage message, Exception exception)
    {
        ...
        if (deadLettered)
        {
            ...
            _logger.LogCritical(
                ...
                typeof(FoodCartDbContext).Name, message.Id, message.Type, message.AttemptCount);
