...
namespace HBA.Communication.Notifications.Infrastructure.Migrations
{
    ...
    /// ═════════════════════════════════════════════════════════════════════════
    /// </summary>
    [DbContext(typeof(HBA.Communication.Notifications.Infrastructure.Persistence.NotificationsDbContext))]
    ...
    public partial class AjoutCorrelationOutbox : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(
                @"ALTER TABLE notifications.outbox_messages ADD COLUMN IF NOT EXISTS ""CorrelationId"" character varying(100);");
        }

        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(
                @"ALTER TABLE notifications.outbox_messages DROP COLUMN IF EXISTS ""CorrelationId"";");
        }
    }
}
