...
namespace HBA.Promotions.Infrastructure.Migrations
{
    ...
    /// ═════════════════════════════════════════════════════════════════════════
    /// </summary>
    [DbContext(typeof(HBA.Promotions.Infrastructure.Persistence.PromotionsDbContext))]
    ...
    public partial class AjoutCorrelationOutbox : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(
                @"ALTER TABLE promotions.outbox_messages ADD COLUMN IF NOT EXISTS ""CorrelationId"" character varying(100);");
        }

        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(
                @"ALTER TABLE promotions.outbox_messages DROP COLUMN IF EXISTS ""CorrelationId"";");
        }
    }
}
