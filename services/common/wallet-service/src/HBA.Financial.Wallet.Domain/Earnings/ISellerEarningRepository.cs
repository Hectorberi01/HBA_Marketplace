...
/// <param name="ProviderFees">Frais du prestataire de paiement.</param>
...
public interface ISellerEarningRepository
{
    ...
    /// <summary>
    /// Gains libérés (escrow levé à la livraison) et donc payables, dans la
    /// période.
    /// </summary>
    ...
    /// <summary>Gains PAYABLES d'un vendeur, DU PLUS ANCIEN AU PLUS RÉCENT.</summary>
    Task<IReadOnlyList<SellerEarning>> ListReleasedBySellerAsync(Guid sellerId, CancellationToken cancellationToken = default);

    /// <summary>
    /// Gains soldés par un retrait donné (pour les rendre payables si le retrait
    /// échoue).
    /// </summary>
