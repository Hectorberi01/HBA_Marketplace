...
namespace HBA.Media.Infrastructure.Messaging.Kafka.Processors;
...
public sealed class OutboxProcessor : BackgroundService
{
    ...
    /// </summary>
    private static readonly string ModuleName = typeof(MediaDbContext).Name.Replace("DbContext", string.Empty);
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                _logger.LogError(ex, "Erreur lors du traitement de l'outbox de {Context}", typeof(MediaDbContext).Name);
    ...
    private void HandleFailure(OutboxMessage message, Exception exception)
    {
        ...
        if (deadLettered)
        {
            ...
            _logger.LogCritical(
                ...
                typeof(MediaDbContext).Name, message.Id, message.Type, message.AttemptCount);
