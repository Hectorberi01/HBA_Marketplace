# recommendation — suggestions de produits
...
> `new RecommendationsModuleInstaller().Install(...)` dans son `Program.cs`.
>
...
| **Schéma** | `recommendations` — isolé, avec ses propres migrations et son propre `DbContext` |
...
* **Recommandations** — « produits similaires », « souvent achetés ensemble ».

`RecommendationType.Similar` et `FrequentlyBoughtTogether` ne sont **jamais
posés** (lot 9.2) : le module stocke un modèle que rien n'alimente.
