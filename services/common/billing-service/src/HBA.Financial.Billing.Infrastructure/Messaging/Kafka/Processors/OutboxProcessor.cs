...
namespace HBA.Financial.Billing.Infrastructure.Messaging.Kafka.Processors;
...
public sealed class OutboxProcessor : BackgroundService
{
    ...
    private static readonly string ModuleName = typeof(BillingDbContext).Name.Replace("DbContext", string.Empty);
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                _logger.LogError(ex, "Erreur lors du traitement de l'outbox de {Context}", typeof(BillingDbContext).Name);
    ...
    private void HandleFailure(OutboxMessage message, Exception exception)
    {
        ...
        if (deadLettered)
        {
            ...
            _logger.LogCritical(
                ...
                typeof(BillingDbContext).Name, message.Id, message.Type, message.AttemptCount);
