...
namespace HBA.Users.Infrastructure.Migrations
{
    ...
    /// ═════════════════════════════════════════════════════════════════════════
    /// </summary>
    [DbContext(typeof(HBA.Users.Infrastructure.Persistence.UsersDbContext))]
    ...
    public partial class AjoutCorrelationOutbox : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(
                @"ALTER TABLE users.outbox_messages ADD COLUMN IF NOT EXISTS ""CorrelationId"" character varying(100);");
        }

        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(
                @"ALTER TABLE users.outbox_messages DROP COLUMN IF EXISTS ""CorrelationId"";");
        }
    }
}
