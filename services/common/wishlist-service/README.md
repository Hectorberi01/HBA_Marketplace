...
>
> Son code s'exécute dans le processus de **review-service**, qui appelle
> `new WishlistModuleInstaller().Install(...)` dans son `Program.cs`.
>
...
| | |
|---|---|
| **Processus** | celui de `services/common/review-service` — même conteneur, même redémarrage, même panne |
| **Base** | `hba_engagement`, partagée avec son hôte |
| **Schéma** | `wishlist` — isolé, avec ses propres migrations et son propre `DbContext` |
...
`Services:Engagement` chez ses appelants, et le remplacement de ses appels en
...
* **Listes d'envies** — un acheteur, des produits mis de côté.

`services/README.md` range encore la wishlist parmi les modules de
`commerce-service` (« Cart, Wishlist, Pricing, Loyalty, Marketing »). Elle est
ici, hébergée par review-service. Ce tableau décrit une cible, pas le dépôt.
