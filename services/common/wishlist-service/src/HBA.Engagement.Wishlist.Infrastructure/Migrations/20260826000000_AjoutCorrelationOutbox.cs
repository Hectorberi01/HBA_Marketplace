...
namespace HBA.Engagement.Wishlist.Infrastructure.Migrations
{
    ...
    /// ═════════════════════════════════════════════════════════════════════════
    /// </summary>
    [DbContext(typeof(HBA.Engagement.Wishlist.Infrastructure.Persistence.WishlistDbContext))]
    ...
    public partial class AjoutCorrelationOutbox : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(
                @"ALTER TABLE wishlist.outbox_messages ADD COLUMN IF NOT EXISTS ""CorrelationId"" character varying(100);");
        }

        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(
                @"ALTER TABLE wishlist.outbox_messages DROP COLUMN IF EXISTS ""CorrelationId"";");
        }
    }
}
